    ...
    <AppContext.Provider value={{ state, setState }}>
      <div className="container mx-auto p-4">
        <div className="flex space-x-4 mb-4">
          <Select onValueChange={value => setSelectedDraftGroup(Number(value))}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder="Select Draft Group" />
            </SelectTrigger>
            <SelectContent>
              {SLATES.map(slate => (
                <SelectItem key={slate.draftGroupId} value={slate.draftGroupId.toString()}>
          ...
          <Select onValueChange={value => setSelectedGame(value)}>
            ...
            <SelectContent>
              {SLATES.find(slate => slate.draftGroupId === selectedDraftGroup)?.games.map(game => (
                <SelectItem key={game.teams} value={game.teams}>
                  {game.teams}
                </SelectItem>
              ))}
            </SelectContent>
